...
def _to_i32(value):
	# zlib.adler32 changed from signed (Python 2) to unsigned (Python 3); restore the
	# signed 32-bit view so values pack into struct "i" fields and match the wire format.
	value &= 0xFFFFFFFF
	if value >= 0x80000000:
		value -= 0x100000000
	return value
...
def _generate_acp_header_key(password):
	...
	Args:
		password (str or bytes): system password of the router (syAP)
	...
	# accept text or bytes; encode text as latin-1 to preserve byte values 1:1
	if isinstance(password, str):
		password = password.encode("latin-1")

	# pad with NULLs
	pw_buf = password[:pw_len].ljust(pw_len, b"\x00")
...
class ACPMessage(object):
	...
	def __init__(self, version, flags, unused, command, error_code, key, body=None, body_size=None):
		...
		if body == None:
			...
			self.body_checksum = _to_i32(zlib.adler32(body))
	...
	def parse_raw(cls, data):
		...
		if header_checksum != _to_i32(zlib.adler32(tmphdr)):
		...
		if body_data and body_checksum != _to_i32(zlib.adler32(body_data)):
